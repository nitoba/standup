...
import type { WeeklyDigestData } from './weekly-digest.js'
import { renderWeeklyDigest, renderWeeklyDigestText } from './weekly-digest.js'
...
describe('renderWeeklyDigest', () => {
  ...
  it('output is under 100KB (anti-spam rule)', () => {
    const html = renderWeeklyDigest(sampleData, LIGHT_THEME)
    const bytes = new TextEncoder().encode(html).length
    expect(bytes).toBeLessThan(100_000)
  })

  it('uses table layout (Outlook compatibility)', () => {
    const html = renderWeeklyDigest(sampleData, LIGHT_THEME)
