...
describe('buildSeedMessages', () => {
  it('returns user, assistant, and toolResult messages', () => {
    const messages = buildSeedMessages({ content: '## Standup\n- item 1', summary: 'Did item 1' })
  ...
  it('assistant message contains submit_standup tool call with correct args', () => {
    const messages = buildSeedMessages({ content: 'my content', summary: 'my summary' })
    const assistant = messages[1] as AssistantMessage
    expect(assistant.role).toBe('assistant')
    const toolCall = assistant.content.find((b): b is ToolCall => b.type === 'toolCall')
