...
vi.mock('@mariozechner/pi-ai', () => ({
  getModel: vi.fn((provider: string, model: string) => `mock-${provider}:${model}`),
}))
...
describe('toPiAiModel', () => {
  ...
  it('maps groq provider correctly', () => {
    const result = toPiAiModel({ provider: 'groq', modelKey: 'groq:llama-3.3-70b-versatile' })
  ...
  it('extracts model name from "provider:model" format with multiple colons correctly', () => {
    ...
    expect(getModel).toHaveBeenCalledWith('openrouter', 'meta-llama/llama-3:latest')
