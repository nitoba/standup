import { ExternalServiceError, Result } from '@standup/domain'

export interface TriggerWeeklyDigestJobDeps {
  workerInternalUrl: string
  internalSecret: string
}

/**
 * Solicita ao worker interno a execução do weekly digest job.
 * A API apenas encaminha o trigger; a execução fica no worker.
 */
export async function triggerWeeklyDigestJob(
  deps: TriggerWeeklyDigestJobDeps,
  userId: string,
): Promise<Result<void, ExternalServiceError>> {
  return Result.tryPromise({
    try: async () => {
      const response = await fetch(
        `${deps.workerInternalUrl}/internal/trigger/weekly-digest`,
        {
          method: 'POST',
          headers: {
            'x-internal-secret': deps.internalSecret,
            'content-type': 'application/json',
          },
          body: JSON.stringify({ userId }),
        },
      )

      if (response.status !== 202) {
        throw new Error(`HTTP ${response.status}`)
      }
    },
    catch: (error) =>
      new ExternalServiceError({
        service: 'worker',
        message: `Failed to trigger weekly digest job: ${error instanceof Error ? error.message : String(error)}`,
      }),
  })
}
