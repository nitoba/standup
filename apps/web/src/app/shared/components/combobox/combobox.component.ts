...
@Component({
  ...
  template: `
    <button
      ...
      #popoverTrigger
    >
      <span class="flex-1 truncate text-left" [class.text-muted-foreground]="!displayValue()" [class.text-foreground]="!!displayValue()">
        {{ displayValue() ?? placeholder() }}
      </span>
      <z-icon zType="chevrons-up-down" class="ml-2 shrink-0 text-muted-foreground opacity-70" />
    </button>
...
export class ZardComboboxComponent implements ControlValueAccessor {
  ...
  protected readonly popoverClasses = computed(() => {
    const widthClass =
      this.zWidth() === 'full'
        ? 'w-full'
        : comboboxVariants({ zWidth: this.zWidth() })
    return `${widthClass} border border-border bg-card p-0 shadow-[0_16px_40px_rgba(0,0,0,0.35)]`
  })
  ...
  handleSelect(commandOption: ZardCommandOption) {
    ...
    const newValue =
      selectedValue === this.currentValue() ? null : selectedValue
