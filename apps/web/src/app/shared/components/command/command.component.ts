...
export class ZardCommandComponent implements ControlValueAccessor, ZardCommand {
  ...
  private readonly optionComponentsAsChildren = contentChildren(
    ZardCommandOptionComponent,
    { descendants: true },
  )
  private readonly registeredOptionComponents = signal<
    ZardCommandOptionComponent[]
  >([])
  ...
  unregisterOption(option: ZardCommandOptionComponent) {
    this.registeredOptionComponents.update((current) =>
    ...
    switch (key) {
      ...
      case 'Enter':
        if (currentIndex >= 0 && currentIndex < filteredOptions.length) {
          ...
          if (selectedOption && !selectedOption.zDisabled()) {
