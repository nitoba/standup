...
@Component({
  ...
  template: `
    <span class="flex items-center gap-3" zardId="switch" #z="zardId">
      <button
        [id]="zId() || z.id()"
        type="button"
        role="switch"
      ...
      >
        <span
          ...
          [attr.data-state]="status()"
          class="pointer-events-none block size-5 rounded-full bg-background shadow-[0_0_0_1px_rgba(255,255,255,0.08)] ring-0 transition-transform data-[size=lg]:size-6 data-[size=sm]:size-4 data-[state=checked]:translate-x-5 data-[size=lg]:data-[state=checked]:translate-x-6 data-[size=sm]:data-[state=checked]:translate-x-4 data-[state=unchecked]:translate-x-0 data-[size=lg]:data-[state=unchecked]:translate-x-0 data-[size=sm]:data-[state=unchecked]:translate-x-0"
        ></span>
      </button>

      <label
        class="font-[var(--font-jetbrains)] text-[13px] leading-none text-foreground peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
        [for]="zId() || z.id()"
      >
